...
class BlogController extends Controller
{
    ...
    public function __construct(Post $post, Category $category, Tag $tag, DatabaseManager $db)
    {
        ...
        $this->middleware('App\Http\Middleware\Blog\ProtectedPost', ['only' => 'show']);
    }
    ...
    {
        $data = $this->getOverviewData();
        $data['posts'] = $this->post->forPublic()
    ...
    public function category($category)
    {
        ...
        $data['posts'] = $this->post->whereCategoryId($category_id)
	        ->forPublic()
	        ->orderBy('publish_date', 'DESC')
	        ->paginate($this->config->items_per_page);
    ...
    {
        $archive = $this->post->where('publish_date', '<=', date('Y-m-d H:i:s'))
	        ->orderBy('publish_date', 'DESC')
	        ->get()
